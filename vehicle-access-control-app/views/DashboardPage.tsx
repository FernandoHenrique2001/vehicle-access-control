...
import {
  ...
  FilterIcon,
  ClockIcon,
} from "../components/icons"; // Added ClockIcon
...
const DashboardPage: React.FC = () => {
  ...
  console.log({ vehicleAccessDistribution });
    ...
    <div className="container mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
        <StatCard
          ...
          value={totalVehicles}
          icon={<CarIcon className="text-white" size={24} />}
          color="bg-blue-500"
        />
        <StatCard
        ...
        <StatCard
          ...
          value={totalAccessesInPeriod}
          icon={<ListIcon className="text-white" size={24} />}
          color="bg-indigo-500"
        />
      </div>
      {/* Stats Cards Row 2 */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <StatCard
          title="Veículos Dentro"
          value={currentEntries}
          icon={<ScanLineIcon className="text-white" size={24} />}
          color="bg-yellow-500"
        />
        <StatCard
          title="Acessos Hoje"
          value={recentEntriesToday}
          icon={<CarIcon className="text-white" size={24} />}
          color="bg-purple-500"
        />
        <StatCard
          ...
          subValue={
            lastAccess
              ? `${lastAccess.vehicleLicense} - ${lastAccess.time}`
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
          ...
          {isLoading && (
            <div className="text-center py-10">Carregando gráfico...</div>
          )}
          {!isLoading && formattedBarChartData.length === 0 && !error && (
            <div className="text-center py-10 text-gray-500">
              Nenhum dado de acesso para o período selecionado.
            </div>
          )}
        ...
        <div className="bg-white p-6 rounded-xl shadow-lg">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">
            Distribuição de Acessos por Veículo
          </h2>
          {isLoading && (
            <div className="text-center py-10">Carregando gráfico...</div>
          )}
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg">
        <h2 className="text-xl font-semibold text-gray-700 mb-4">
          Mapa de Calor de Atividade (Dia/Hora)
        </h2>
        {isLoading && (
          <div className="text-center py-10">Carregando mapa de calor...</div>
        )}
        {!isLoading && accessHeatmapGrid.maxValue === 0 && !error && (
          <div className="text-center py-10 text-gray-500">
            Sem atividade para exibir no mapa de calor.
          </div>
        )}
        {!isLoading && accessHeatmapGrid.maxValue > 0 && (
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse border border-gray-300 text-center">
              <thead>
                <tr>
                  <th className="p-2 border border-gray-300 bg-gray-100 text-xs w-16">
                    Hora
                  </th>
                  {accessHeatmapGrid.days.map((day) => (
                    <th
                      key={day}
                      className="p-2 border border-gray-300 bg-gray-100 text-xs"
                    >
                      {day}
                    </th>
                  ))}
              ...
              <tbody>
                {accessHeatmapGrid.hours.map((hour, hourIndex) => (
                  <tr key={hour}>
                    <td className="p-1.5 border border-gray-300 bg-gray-50 text-xs font-medium w-16">
                      {hour}
                    </td>
