  ...
  const loadDashboardData = useCallback(
    async (filters?: { startDate?: string; endDate?: string }) => {
      ...
      try {
        ...
        if (dashData.entries.length > 0) {
          dashData.entries.forEach((entry) => {
            ...
            if (!latestAccess || entryEventTime > latestAccess.rawTime) {
              latestAccess = {
                vehicleLicense: entry.vehicle?.license || "N/A",
        ...
        dashData.entries.forEach((entry) => {
          const license = entry.vehicle?.license || "Desconhecido";
        ...
        if (sortedVehicles.length > TOP_N_VEHICLES) {
          ...
          const otherValue = sortedVehicles
            .slice(TOP_N_VEHICLES)
            .reduce((sum, current) => sum + current.value, 0);
          if (otherValue > 0) {
            vehicleAccessDistribution.push({
              name: "Outros",
              value: otherValue,
            });
          }
        ...
        const entriesTodayCount = dashData.entries.filter(
          (entry) =>
            new Date(entry.entryTime).toISOString().split("T")[0] === today
        ).length;
        const currentInsideCount = dashData.entries.filter(
